...
export const Settings: FC<Props> = ({onCloseSystemSetting, onChangeLanguage, language, apiKey, onApiKeyChange}) => {
  ...
  <div className="overflow-none relative p-10 flex-1 bg-white dark:bg-[#343541]">
    <div className="mx-auto flex h-full pt-10 flex-col space-y-6">
      ...
      <div>
        <label htmlFor="apiKey" className="block text-sm font-medium leading-6 text-gray-800  dark:text-gray-100">{ t('api Key') }</label>
        <div className="mt-2">
          <input type="text" name="apiKey" id="apiKey" value={apiKey} onChange={(e) => onApiKeyChange(e.target.value)} className="block w-full rounded-md border-0 py-1.5 border border-neutral-600 bg-[#202123] px-4 py-3 pr-10 text-[12px] leading-3 text-white" />
        </div>
      </div>
      {/* <div>
        <label htmlFor="first-name" className="block text-sm font-medium leading-6 text-gray-800  dark:text-gray-100">{t('Use Bridge')}</label>
        <div className="mt-2">
