...
const Home: React.FC<HomeProps> = ({ serverSideApiKeyIsSet }) => {
  ...
  const handleSend = async (message: Message, deleteCount = 0) => {
    if (selectedConversation) {
      ...
      const response = await fetch(`${DEFAULT_BIRDGE_URL}/api/chat`, {
        method: 'POST',
        headers: {
  ...
  const fetchModels = async (key: string) => {
    ...
    const response = await fetch(`${DEFAULT_BIRDGE_URL}/api/models`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
            ...
            {showSidebar ? (
              <div>
                <Sidebar
                  ...
                  onSystemSetting={handleSystemSetting}
                ...
                <IconArrowBarLeft
                  className="fixed top-2 left-[270px] z-50 h-7 w-7 cursor-pointer hover:text-gray-400 dark:text-white dark:hover:text-gray-300 sm:top-2 sm:left-[220px] sm:h-5 sm:w-5 sm:text-neutral-700"
              ...
              <IconArrowBarRight
                className="fixed top-2 z-50 h-7 w-7 cursor-pointer text-white hover:text-gray-400 dark:text-white dark:hover:text-gray-300 sm:top-2 sm:left-20 sm:h-5 sm:w-5 sm:text-neutral-700"
                onClick={() => setShowSidebar(!showSidebar)}
              />
            )}
